function [ops_contribs, new_full_states] ...
    = InvOps(params, t, state_list, decision_list, uncertainty_list, full_state_list) %#ok<INUSD,INUSL> extra params listed by name for clarity
...
%   1  2016-04-14 11:23  BryanP      Initial Code that uses existing InvTransProb v4 

if nargin < 4 || isempty(decision_list)
    % Before Decision -- nothing to do
    warning('DpInv:UnusedOps','Unused Operations call before decision. Only after decision operations used in Multi-Inventory')
...
elseif nargin < 5 || isempty(uncertainty_list)
    ...
    warning('DpInv:UnusedOps','Unused Operations after random. Only after decision operations used in Multi-Inventory')
    ops_contribs = zeros(size(state_list,1), 1);
end

function varargout = csv2cell( fileName, varargin )
% CSV2CELL reads the specified CSV file and stores the contents in a cell array or matrix
%
...
%
% WARNINGS (added by BP):
%  - You must terminate csv file with a blank line or this code will
%    miss the last line
%  - MATLAB has some internal function called csv2cell that will be called
%    if this function is not in your path... ergh
%
% CSV2CELL( fileName ):         fileName     -  String specifying the CSV file to be read. Set to
%                                                [] to interactively select the file.
%
% CSV2CELL( fileName, ... ) : Specify a list of options to be applied when importing the CSV file.
...
%
% [C1 C2 C3] = CSV2CELL( fileName, 'columns', {'C1', 'C2', C3'}, ... )
...
%
% [C1 C2 C3] = CSV2CELL( fileName, 'columns', [2, 3, 4], ,'noHeader', true, ... )
%   This form returns the data from columns in output variables C1, C2 and C3 respectively, the
%   columns parameter must contain the column indices when the 'noHeader' option is set to true.
%
% by Ashish Sadanandan from MATLAB Central File Exchange
% BSD Liscense
...
%   05/20/2010 (BryanP): Renamed to csv2cell for naming consistancy
%   11/17/2013 (BryanP): Add support for files without trailing newlines
%
...
if ( nargin == 0 ) || isempty( fileName )
  ...
  if ~ischar( fileName )
    error( 'csv2cell:FileNameError', 'The first argument to %s must be a valid .csv file', ...
      mfilename );
  end
end
...
if nargin > 1
  if mod( numel( varargin ), 2 ) ~= 0
    error( 'csv2cell:InvalidInput', ['All input parameters after the fileName must be in the ' ...
  ...
  for ii =  1 : numel( params )
    ...
    if numel( result ) > 1
      ...
      if ~isempty( exresult )
        ...
        error( 'csv2cell:InvalidInput', 'Cannot find unambiguous match for parameter ''%s''', ...
          varargin{ii*2-1} );
      end
    end
...
if isempty( p.delimiter ) || ~ischar( p.delimiter )
  error( 'csv2cell:InvalidParamType', ['The ''delimiter'' parameter must be a non-empty ' ...
    'character array'] );
end
if isempty( p.noHeader ) || ~islogical( p.noHeader ) || ~isscalar( p.noHeader )
  error( 'csv2cell:InvalidParamType', ['The ''noHeader'' parameter must be a non-empty ' ...
    'logical scalar'] );
end
if ~p.noHeader
  if ~isempty( p.columns )
    if ~ischar( p.columns ) && ~iscellstr( p.columns )
      error( 'csv2cell:InvalidParamType', ['The ''columns'' parameter must be a character array ' ...
  ...
  if ~isempty( p.columns ) && ~isnumeric( p.columns )
    error( 'csv2cell:InvalidParamType', ['The ''columns'' parameter must be a numeric array ' ...
...
if isempty( p.outputAsChar ) || ~islogical( p.outputAsChar ) || ~isscalar( p.outputAsChar )
  error( 'csv2cell:InvalidParamType', ['The ''outputAsChar'' parameter must be a non-empty ' ...
    'logical scalar'] );
end
if isempty( p.uniformOutput ) || ~islogical( p.uniformOutput ) || ~isscalar( p.uniformOutput )
  error( 'csv2cell:InvalidParamType', ['The ''uniformOutput'' parameter must be a non-empty ' ...
    'logical scalar'] );
end
...
if fid == -1
  error( 'csv2cell:FileReadError', 'Failed to open ''%s'' for reading.\nError Message: %s', ...
    fileName, msg );
end
...
if colMode
  if ( nargout > 1 ) && ( nargout ~= nHeaders )
    error( 'csv2cell:NumOutputs', ['The number of output arguments must be 1 or equal to the ' ...
...
if colMode
  if ~p.noHeader
    ...
    for ii = 1 : nHeaders
      ...
      if isempty( result )
        fclose( fid );
        error( 'csv2cell:UnknownHeader', ['Cannot locate column header ''%s'' in the file ' ...
      ...
      elseif numel( result ) > 1
        ...
        if numel( exresult ) == 1
          ...
          warning( 'csv2cell:MultipleHeaderMatches', ['Column header name ''%s'' matched ' ...
    ...
    if max( colIdx ) > nCols
      fclose( fid );
      error( 'csv2cell:BadIndex', ['The specified column index ''%d'' exceeds the number of ' ...
...
if pos == -1
  msg = ferror( fid );
  fclose( fid );
  error( 'csv2cell:FileQueryError', 'FTELL on file ''%s'' failed.\nError Message: %s', ...
    fileName, msg );
end
data            = fread( fid );
nLines          = numel( find( data == sprintf( '\n' ) ) ) + 1;

%BSP: Add support for files without trailing newlines
if data(end) ~= sprintf('\n')
    nLines = nLines +1;
end
...
if fseek( fid, pos, 'bof' ) ~= 0
  msg = ferror( fid );
  fclose( fid );
  error( 'csv2cell:FileSeekError', 'FSEEK on file ''%s'' failed.\nError Message: %s', ...
    fileName, msg );
end
...
for ii = 2 : nLines
  ...
  if nDataElems < nCols
    warning( 'csv2cell:UnevenColumns', ['Number of data elements on line %d (%d) differs from ' ...

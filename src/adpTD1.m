...
adp = DefaultOpts(varargin, adp_defaults);
...
[adp] = InitOptions(adp, problem);
[adp, vfun, iter_start] = InitVfun(adp, problem);
...
if not(isempty(adp.vfun_init_alg))
    vfun = SeedValueFunctions(vfun, adp, problem, results);
end
...
for iter = (iter_start+1):(iter_start + adp.max_iter)
    ...
    start_post = adp.fn.ApplyDscn(problem, pre_start, 0, t_start);
...
%                 contrib = sim_contrib(backpass_ok, t+1);
            ...
            if adp.bkps_use_updated_vfun
                ...
                actual_vals = vfun(t).approx(vfun_state, adp.vfun_approx_params{:});
            ...
            if adp.bkps_abort && not(in_bootstrap)
                ...
                %Note: no decisions in the terminal state: will only abort those on values
                    ...
                    new_backpass_ok(backpass_ok) = adp.fn.ValCompare(new_vals, fwd_val(backpass_ok, t), adp.tol_bkps_val);
...
results.n_iter = iter;
[results.first_desc, dec_contrib, ~, cost_to_go] = adp.fn.OptimalDec(problem, 1, first_pre_state, vfun(1), adp);
...
%%   InitOptions
%----------------------
function[adp] = InitOptions(adp, problem)
    ...
    adp.fn.OptimalDec = FunctForProblem(problem, 'fOptimalDec', @FindOptDecFromVfun);
    adp.fn.DecCompare = FunctForProblem(problem, 'fDecCompare', @isequaln);
    adp.fn.ValCompare = FunctForProblem(problem, 'fValCompare', @RelValCompare);
...
%%   InitVfun
%----------------------
function [adp, vfun, iter_start] = InitVfun(adp, problem)
...
%   Seed Value Functions
%----------------------
function vfun = SeedValueFunctions(vfun, adp, problem, results)
    % If requested Initial Value function approximation using start_algorithm
    start_alg = str2func(adp.seed_vfun_init);

    % Run the starting algorithm
    if adp.verbose
        fprintf('Seeding value functions using %s', adp.seed_vfun_init)
    end
    
    %Configure to use existing value functions
    % Note: structures are passed by value in MATLAB
    results.vfun = vfun;
    adp.old_results = results;
    adp.vfun_update_in_place = true;    %Use existing value functions
    
    % Actually run the startup algorithm
    seed_results = start_alg(problem, adp);
...
function [decision, dec_contrib, post_s, fwd_val, pre_s_list, sim_contrib] ...
            = ForwardPassCore(problem, vfun, adp, ...
                              t_start, pre_start, ...
...
% "Insides" of ForwardPass to be shared by for & parfor loops
    ...
    for t = t_start:problem.n_periods
        ...
        if in_bootstrap && (t==t_start)
            ...
            [decision(t, :), dec_contrib(t), post_s(t, :), fwd_val(t)] ...
                = adp.fn.OptimalDec(problem, t, pre_s, vfun(t), adp);
        end
        ...
        [pre_s_list(t+1, :), sim_contrib(t), full_state]...
            = adp.fn.Sim(problem, t, decision(t, :), post_s(t, :), full_state);
    ...
    % directly
    post_s(t, :)...
        = adp.fn.ApplyDscn(problem, pre_s_list(t, :), 0, t);
    %Compute forward values
    if isempty(adp.fn.PostToVfun)
        vfun_state_list = post_s(t, :);
    else
        vfun_state_list = adp.fn.PostToVfun(problem, post_s(t, :), t);
    end
    ...
    [~, sim_contrib(t), ~]...
        = adp.fn.Sim(problem, t, 0, post_s(t, :), full_state);
...
function new_backpass_ok = ...
    BackPassDecCheckCore(problem, vfun, adp, t, pre_s, old_dec)
    new_dec = adp.fn.OptimalDec(problem, t, pre_s, vfun, adp);
    if not(adp.fn.DecCompare(new_dec, old_dec))
